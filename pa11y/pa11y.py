...
try:
    ...
    if makefile == "use-default":
        if os.path.exists(os.path.join(args.working_dir, "Makefile.sp")):
            ...
            pa11y_target = "sp-" + pa11y_target
        else:
            makefile = "Makefile"

    # Install the doc framework and run the accessibility check
    run_command(f"make -f {makefile} {install_target}", args.working_dir)
    run_command(f"make -f {makefile} {pa11y_target}", args.working_dir)
